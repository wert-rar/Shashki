...
function startPolling() {
  setInterval(() => server_update_request(CURRENT_STATUS, pieces), 100);
}
// SERVER REQUEST CODE
...
function onMouseDown(evt) {
  ...
  if (SELECTED_PIECE) {
    SELECTED_PIECE.offsetX =
      loc.x -
      (BOARD_OFFSET_X + CELL_SIZE * (SELECTED_PIECE.piece.x + 0.5));
    SELECTED_PIECE.offsetY =
      loc.y -
      (BOARD_OFFSET_Y + CELL_SIZE * (SELECTED_PIECE.piece.y + 0.5));
    IS_SELECTED = true;
  }
}
...
function onMouseMove(evt) {
  ...
  if (SELECTED_PIECE != null && IS_SELECTED) {
    let rect = CANVAS.getBoundingClientRect();
    SELECTED_PIECE.x =
      evt.clientX - rect.left - SELECTED_PIECE.offsetX;
    SELECTED_PIECE.y =
      evt.clientY - rect.top - SELECTED_PIECE.offsetY;
  }
}
...
function onTouchMove(evt) {
  ...
  if (SELECTED_PIECE != null && IS_SELECTED && evt.touches.length > 0) {
    ...
    SELECTED_PIECE.x = loc.x;
    SELECTED_PIECE.y = loc.y;
  }
}
